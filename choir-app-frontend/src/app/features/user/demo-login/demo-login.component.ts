  ...
  template: `
    <div class="demo-login-container">
      @if (loading) {
        ...
        <p>Demo-Zugang wird hergestellt…</p>
      }
      @if (error) {
        <mat-icon class="error-icon">error_outline</mat-icon>
        <h2>Demo-Zugang fehlgeschlagen</h2>
        <p>{{ error }}</p>
...
export class DemoLoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.authService.consumeDemoLead(token).subscribe({
      next: () => {
        this.router.navigate(['/dashboard']);
      },
      error: (err) => {
        this.loading = false;
        this.error = err.error?.message || 'Demo-Zugang ungültig oder abgelaufen.';
      },
    });
  }
}
